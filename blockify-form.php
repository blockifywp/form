<?php
/**
 * Plugin Name: Blockify Forms
 * Plugin URI:  https://blockifywp.com/blocks/form
 * Description: Lightweight, customizable form block for WordPress.
 * Author:      Blockify
...
function create_user_from_form(): void {
	...
	if ( $insert_user && ! get_user_by( 'email', $user_email ) && isset( $user_data['user_login'] ) ) {
		$user = wp_insert_user( apply_filters( 'blockify_insert_user_data', $user_data ) );
...
add_filter( 'blockify', NS . 'add_plugin_editor_vars' );
/**
...
function add_plugin_editor_vars( array $config ): array {
	...
	$config['googleRecaptcha'] = get_option( 'blockify' )['googleRecaptcha'] ?? '';
